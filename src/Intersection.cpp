...
void WaitingVehicles::permitEntryToFirstInQueue()
{
    ...
    std::shared_ptr<Vehicle> front_in_queue = _vehicles.front();
    std::promise<void> promise_front_in_queue = std::move(_promises.front());
    promise_front_in_queue.set_value();
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle)
{
    ...
    //  First, add the new vehicle to the waiting line by creating a promise, a corresponding future and then adding both to _waitingVehicles. 
    // Then, wait until the vehicle has been granted entry. 
    std::promise<void> promise_entry;
    std::future<void> future_entry = promise_entry.get_future();
    std::thread t(&WaitingVehicles::pushBack, std::ref(_waitingVehicles), vehicle, std::move(promise_entry));
    future_entry.wait();
    t.join();
